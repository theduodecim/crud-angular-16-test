...
export class NewHeroDialogComponent implements OnInit {
  ...
  heroForm!: FormGroup;
  statuses: any[] = ['success', 'info', 'warning', 'danger'];
  ...
  onAutocompleteChange(event: AutoCompleteCompleteEvent) {
    ...
    for (let i = 0; i < (this.arr_heros as Hero[]).length; i++) {
      ...
      if (hasSymbols) {
        ...
        if (outputText?.toUpperCase().includes(query.toUpperCase()))
          filtered.push(hero);
      } else {
        if (hero.name?.toUpperCase().includes(query.toUpperCase()))
          filtered.push(hero);
      }
    }
  ...
  loadHeroData(hero: Hero) {
    ...
    var categoryFound = this.arr_category_options.find(
      (f) => f.label == hero?.category
    );
